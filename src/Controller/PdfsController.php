...
class PdfsController extends AppController {
    ...
    public function relatorioEquipamento($equipamento, $dataInicio, $dataFim){
		...
		$pdf->SetCreator(PDF_CREATOR);
		$pdf->SetAuthor("SGL");
		$pdf->SetTitle('Relatório de Alertas - '.$equipamento->nome.'[Tombo: '.$equipamento->tombo.'] - '.date('dmY', strtotime($dataInicio)).' - '.date('dmY', strtotime($dataFim)));
		$pdf->SetSubject('');
		...
		$pdf->relatorioEquipamento($equipamento, $dataInicio, $dataFim);

		

		return $this->response->file(
            $pdf->Output('Relatório de Alertas - '.$equipamento->nome.'[Tombo: '.$equipamento->tombo.'] - '.date('dmY', strtotime($dataInicio)).' - '.date('dmY', strtotime($dataFim)).'.pdf', 'I')
        );
